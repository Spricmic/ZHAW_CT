...
int main(void)
{
	...
	while(1){
		...
		temp = adc_get_value(resolution);
		temp = normalize_value(temp, resolution);
		//display_on_lcd(temp);
		durchschnitt = adc_filter_value(temp);
		display_on_lcd(durchschnitt);
		///CT_SEG7->BIN.HWORD = adc_filter_value(temp);
	}
...
static uint16_t normalize_value(uint16_t value, adc_resolution_t resolution)
{
    ...
    /// STUDENTS: To be programmed
		...
		if(resolution == ADC_RES_6BIT){ 
			normalized = (value * 3300) / 0x3F;
		}
		else if(resolution == ADC_RES_8BIT){ 
			normalized = (value * 3300) / 0xFF;
		}
		else if(resolution == ADC_RES_10BIT){ 
			normalized = (value * 3300) / 0x3FF;
		}
		else if(resolution == ADC_RES_12BIT){ 
			normalized = (value * 3300) / 0xFFF;
		}
		else{
			normalized = 0x0;
		}
