...
int main(void)
{
    ...
    while (1){
        ...
        NVIC->IP[IRQNUM_TIM2] = 0x20;      //set priority level of timer2 to 1
...
{
    /// STUDENTS: To be programmed
		uint32_t time_interrupt = TIM2->CNT;  // take time when interrupt happend
		hal_timer_irq_clear(TIM2, HAL_TIMER_IRQ_UE);
		tim2_interrupt_counter += 1; // increment interrupt counter 
		// update variables
		sum_latency += time_interrupt;
		if (time_interrupt < min_latency){
			min_latency = time_interrupt;
		}
		if (time_interrupt > max_latency){
			max_latency = time_interrupt;
		}
		// stop tests if amount of needed test has been run.
		if(tim2_interrupt_counter >= NUMBER_OF_TIMER_2_INTERRUPTS){
			hal_timer_stop(TIM2);
			hal_timer_stop(TIM3);
			measurement_done = TRUE;
		}
		uint32_t time_interrupt_end = TIM2->CNT;
		sum_tisr += time_interrupt_end - time_interrupt;
...
static void print_results(void)
{
    ...
    /// STUDENTS: To be programmed
		sprintf(ret_val, "%s%d", label_min, min_latency);
		hal_ct_lcd_write(pos, ret_val);
		pos += strlen(ret_val)+1;
		
		sprintf(ret_val, "%s%d", label_max, max_latency);
		hal_ct_lcd_write(pos, ret_val);
		pos += strlen(ret_val)+1;
		
		sprintf(ret_val, "%s%d", lable_tsr, (sum_latency/tim2_interrupt_counter));
		hal_ct_lcd_write(pos, ret_val);
		pos += strlen(ret_val)+1;
		
		sprintf(ret_val, "%s%d", label_avg, avg_latency);
		hal_ct_lcd_write(pos, ret_val);
		pos += strlen(ret_val)+1;
		
		sprintf(ret_val, "%s%d", label_load, tim3_interrupt_counter);
		hal_ct_lcd_write(pos, ret_val);
		pos += strlen(ret_val)+1;
